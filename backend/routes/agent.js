...
//
// 业务流程定位：
//   QC 阶段（quality_engineer）→ 外观检查，不涉及循环测试数据
//   Under Test 阶段（lab_operator）→ 循环充放电测试，记录 metrics，触发 Agent 分析
//
// 因此：
//   ① 只有 lab_operator 能触发 Agent 分析
//   ② 电池必须处于 Under Test 状态，否则拒绝分析
router.post('/analyze/:cellCode', verifyToken, requireRole('lab_operator'), async (req, res) => {
  ...
  try {
    ...
    // ── 改动1：状态检查，只允许 Under Test 状态触发分析 ──
    if (cell.current_state !== 'Under Test') {
      return res.status(400).json({
        success: false,
        error_code: 'INVALID_CELL_STATE',
        message: `Agent analysis requires cell to be in 'Under Test' state. Current state: '${cell.current_state}'. Please advance the cell through the workflow first.`,
        report
      });
    }

    // ── Step 2: 查询测试数据（按 cycle_count 排序）────────
